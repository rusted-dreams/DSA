...
int findDuplicate(vector<int> &arr){
    ...
    // X-ORing every no. of arr with each other hence the no.that comes twice will become zero
    for(int i=0; i<arr.size(); i++){
        res ^= arr[i];
    }


    /*X-ORing the res which has every no. other than the duplicate with the nos. 1-n-1 
    which will make every no. zero which was once and we will remain with the no.which appeared twice. 
    since it became 0 on the previous step now it is the only no. left and hence returned.
    */
    for(int i=0; i<arr.size(); i++){
        res ^= i;
    }
